...
export default function Login() {
  ...
  const { data: business } = useQuery<BusinessProfile>({
    ...
    queryFn: () => api.get(`/settings/business${workspaceSlug ? `?slug=${encodeURIComponent(workspaceSlug)}` : ''}`).then((r) => r.data),
  });
  ...
  return (
    <div className="login-page">
      <div className="login-card">
        {/* Logo */}
        <div className="login-logo" style={{ overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-input)' }}>
          {business?.logo ? (
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="form-group" style={{ position: 'relative' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
              ...
              <button
                type="button"
                onClick={() => alert('Please contact your business administrator (Owner) to reset your password from the Staff Management settings dashboard.')}
                style={{ background: 'none', border: 'none', color: 'var(--accent-blue)', fontSize: 11, cursor: 'pointer', fontWeight: 600, padding: 0 }}
              >
                Forgot Password?
              </button>
            </div>
